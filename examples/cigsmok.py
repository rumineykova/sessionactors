...
def start_protocol_old():
    ...
    from cell.examples import smoker
    from cell.examples.cigsmok import Smoker, Arbitrer
    agent = dAgent(Connection())
    arb = agent.spawn(Arbitrer)
    for i in range(0, SmokersConfig.numSmokers):
        agent.spawn(Smoker, id = get_group_id(Smoker, i))
    import time
    time.sleep(3)
    arb.call.join(numRounds=3).result()