...
def start_protocol_old():
    ...
    from cell.examples.barber import Room, Customer, Selector,Barber
    agent = dAgent(Connection())
    room = agent.spawn(Room)
    barber = agent.spawn(Barber)
    selector = agent.spawn(Selector)
    for i in range(0, BarberConfig.customers):
        agent.spawn(Customer, id = get_group_id(C, i))
    import time
    time.sleep(5)
    room.call.join().result()
    selector.call.join().result()
    selector.call.start()