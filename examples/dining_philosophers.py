...
def start_protocol_old():
    ...
    from cell.examples import dining_philosophers
    from cell.examples.dining_philosophers import Philosopher, Arbitrator
    agent = dAgent(Connection())
    arb = agent.spawn(Arbitrator)
    import time
    time.sleep(4)
    arb.call.join().result()
    phils = []
    for i in range(0, PhilConfig.numForks):
        phil = agent.spawn(Philosopher, id = get_group_id(Ph, i))
        time.sleep(2)
        phil.call.join(num = i).result()
        phils.append(phil)
    time.sleep(2)
    [phil.call.start() for phil in phils]